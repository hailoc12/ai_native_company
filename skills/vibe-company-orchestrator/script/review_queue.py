#!/usr/bin/env python3
"""
vibe-company-orchestrator review_queue.py

Scans output/ for company artifacts (OKR, Quality Standards, handoff brief)
with need_review=true or low confidence_score, collects them into
output/review-queue.md for CEO/founder approval.

Why: OKR/SLA targets with low confidence (esp. Stretch x10 OKR, external SLA)
must be human-verified before commit — never auto-accept a hallucinated target.
...
def write_queue(items: list[dict], queue_path: Path) -> None:
    ...
    header = f"# Review Queue — Updated {datetime.now(timezone.utc).isoformat(timespec='minutes')}Z\n\n"
...
def main():
    parser = argparse.ArgumentParser(description="vibe-company-orchestrator review queue")
